...
fn track_namespace_to_python(py: Python<'_>, namespace: &TrackNamespace) -> PyResult<Py<PyList>> {
...
/// メッセージ種別を表す文字列を返す。
fn message_kind(message: &ControlMessage) -> &'static str {
    match message {
...
fn message_body_to_python(py: Python<'_>, message: &ControlMessage) -> PyResult<Py<PyDict>> {
    let dict = PyDict::new(py);
    match message {
...
impl CoreEvent {
    ...
    fn __repr__(&self) -> String {
        format!("_CoreEvent(kind={:?})", self.kind)
    }
}
...
#[pyfunction]
fn classify_data_stream_type(type_id: u64) -> Option<&'static str> {
    shiguredo_moqt::stream::classify_data_stream_type(type_id).map(
...
#[pyfunction]
fn setup_stream_type() -> u64 {
    shiguredo_moqt::stream::SETUP_STREAM_TYPE
}
...
#[pyfunction]
fn is_padding_datagram(data: &[u8]) -> PyResult<bool> {
    match decode_varint_prefix(data).map_err(runtime_error)? {
        Some((type_id, _)) => Ok(type_id == shiguredo_moqt::stream::PADDING_DATAGRAM_TYPE),
        None => Ok(false),
    }
}
...
mod _native {
    ...
    use shiguredo_moqt::stream::{
        FETCH_HEADER_TYPE, PADDING_DATAGRAM_TYPE, PADDING_STREAM_TYPE, SETUP_STREAM_TYPE,
    };
    ...
    #[pymodule_export]
    use crate::{classify_data_stream_type, is_padding_datagram, setup_stream_type};
